...
Int_t LKDrawingGroup::Write(const char *name, Int_t option, Int_t bsize) const
{
    ...
    {
        auto numSub = GetNumGroups();
        cout << "Writting " << numSub << " groups in " << fName << " (" << GetNumAllDrawingObjects() << ")" << endl;
        for (auto iSub=0; iSub<numSub; ++iSub) {
            auto sub = (LKDrawingGroup*) At(iSub);
            LKMisc::AddOption(name0,"draw_count",countDrawings);
...
void LKDrawingGroup::Print(Option_t *opt) const
{
    ...
    if (LKMisc::CheckOption(option,"level")==false) {
        isTop = true;
        cout << "LKDrawingGroup " << fName << endl;
    ...
    {
        auto numDrawings = GetEntries();
        cout << header << "Group " << fName << " containing " << numDrawings << " drawings" << endl;
        if (print_all) {
            cout << "  - level=" << fGroupLevel << endl;
            cout << "  - div=(" << fDivX << "," << fDivY << ")" << endl;
            cout << "  - option=" << fGlobalOption << endl;
        }
        for (auto iDrawing=0; iDrawing<numDrawings; ++iDrawing)
        {
            auto drawing = (LKDrawing*) At(iDrawing);
        ...
        if (LKMisc::CheckOption(option,"!drawing")==false) cout << endl;
    }
    if (isTop)
        cout << GetNumAllDrawings() << " drawings in total" << endl;
}
    ...
    if ((fDivX==0 || fDivY==0) || numDrawingsChange>0)
    {
        if (CheckOption("wide_canvas"))
        {
            if      (numDrawings== 1) { fDivX =  1; fDivY =  1; }
                ...
                cout << "Too many drawings!!! " << numDrawings << endl;
                return false;
            }
        }
    ...
    if (!groupSelection.IsNull())
    {
        ...
        for (auto groupName : groupNameArray)
        {
            if (groupName.EndsWith("*"))
            {
                ...
                while ((key=(TKey*)nextKey())) {
                    if (TString(key->GetName()).Index(groupName)==0) {
                        ...
                        if (allowPrint) cout << "Adding " << group->GetName() << " from " << file->GetName() << endl;
            ...
            else
            {
                ...
                if (obj==nullptr) {
                    cout << groupName << " is nullptr" << endl;
                    continue;
                }
                if (TString(obj->ClassName())=="LKDrawingGroup") {
                    ...
                    cout << groupName << " type is not LKDrawingGroup" << endl;
                    return false;
                }
            }
        }
    ...
    else
    {
        ...
        while ((key=(TKey*)nextKey())) {
            if (TString(key->GetClassName())=="LKDrawingGroup") {
                auto group = (LKDrawingGroup*) key -> ReadObj();
                if (allowPrint) cout << "Adding group (" << group->GetName() << ") from " << file->GetName() << endl;
                AddGroupInStructure(group);
            }
            else if (TString(key->GetClassName())=="LKDrawing") {
                auto draw = (LKDrawing*) key -> ReadObj();
                if (allowPrint) cout << "Adding drawing (" << draw->GetName() << ") from " << file->GetName() << endl;
                if (sub==nullptr)
                    sub = CreateGroup("sub");
                sub -> AddDrawing(draw);
            }
                ...
                if (allowPrint) cout << "Adding TObjArray (" << array->GetName() << ") from " << file->GetName() << endl;
                if (sub==nullptr)
                    sub = CreateGroup("sub");
...
bool LKDrawingGroup::AddFile(TString fileName, TString groupSelection)
{
    ...
    if (!file->IsOpen()) {
        cout << fileName << " cannot be openned" << endl;
        return false;
    }
...
bool LKDrawingGroup::CheckIsGroupGroup(bool add)
{
    ...
    if (!fIsGroupGroup) {
        if (add) cout << "group is drawing-group" << endl;
        return false;
    }
    return true;
}
...
bool LKDrawingGroup::CheckIsDrawingGroup(bool add)
{
    ...
    if (fIsGroupGroup) {
        if (add) cout << "group is group-group" << endl;
        return false;
    }
    return true;
}
...
void LKDrawingGroup::Add(TObject *obj)
{
    ...
    if (obj->InheritsFrom(LKDrawing::Class()) || obj->InheritsFrom(LKDrawingGroup::Class()))
        ...
        cout << "Cannot add " << obj->ClassName() << " directly!" << endl;
}

int LKDrawingGroup::HAdd(TString fileName)
{
    cout << "Adding histograms in " << fileName << " (";
    int numHistograms = HAdd(new LKDrawingGroup(fileName,"xprint"));
    cout << numHistograms << ")" << endl;
