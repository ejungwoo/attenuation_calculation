...
{
    if (iObj>GetEntries()) {
        cout << "index " << iObj << " out of limit " << GetEntries() << endl;
        return;
    }
...
{
    if (obj==nullptr) {
        cout << "trying to add nullptr to " << fName << endl;
        return -1;
    }
...
{
    if (fMainHist==nullptr) {
        cout << "Main histogram is not set!" << endl;
        return;
    }
...
void LKDrawing::SetFitObjects(TObject *dataObj, TF1 *fit)
{
    ...
    if (idxData<0) { cout << "Given data object do not exist in the list!" << endl; return; }
    if (idxFit <0) { cout << "Given function object do not exist in the list!" << endl; return; }
...
TH2D* LKDrawing::MakeGraphFrame(double x1, double x2, double y1, double y2, TString name0, TString title0)
{
    ...
    if (CheckOption("debug_draw"))
        cout << name << " " << title << " " << nx << " " << x1 << " " << x2 << " " << ny << " " << y1 << " " << y2 << endl;
...
void LKDrawing::MakeLegend(bool remake)
{
    ...
    for (auto iObj=0; iObj<numObjects; ++iObj)
    {
        ...
        {
            if (debug_draw) cout << "legend-entry: [" << obj->ClassName() << "] | " << title << " | " << drawOption << " -> " << legendOption << endl;
            fLegend -> AddEntry(obj,title,legendOption);
        }
        if (TString(obj->ClassName())=="TObject")
        {
            if (debug_draw) cout << "legend-entry : [Line] | " << title << " | " << drawOption << " -> " << legendOption << endl;
...
void LKDrawing::Draw(Option_t *option)
{
    ...
    if (fGlobalOption.Index("0:")==0) {
        if (debug_draw)
            cout << "Skipping because of SetDraw(false) option" << endl;
        return;
    }
    ...
    if (debug_draw)
        cout << "Draw option: " << option << endl;

    if (numObjects==0) {
        cout << "empty drawing" << endl;
        //fRM -> StopAll();
        return;
    }
    if (debug_draw) cout << "Number of objects in draiwng: " << numObjects << endl;
    //fRM -> Stop(3);
    ...
    if (fCvs==nullptr) {
        ...
        if (CheckOption("debug_draw"))
            cout << fCvs->GetName() << " " << fCvs->GetTitle() << " " << fCvs->GetWw() << " " << fCvs->GetWh() << endl;
    }
    //fRM -> Stop(4);

    //fRM -> Start(5);
    bool optimize_legend_position = (CheckOption("legend_below_stats") || CheckOption("legend_corner"));
    if (debug_draw) cout << "Optimize legend position?: " << optimize_legend_position << endl;
    ...
    {
        if (debug_draw) cout << "Make color distinguishable 2d-histograms" << endl;
        countHistCC = 1;
        for (auto iObj=0; iObj<numObjects; ++iObj) {
            auto obj = At(iObj);
    ...
    if (debug_draw) cout << "Canvas Margin: " << ml << ", " << mr << ", " << mb << ", " << mt << endl;
    double x1 = 0;
    double x2 = 100;
    double y1 = 0;
    double y2 = 100;
    ...
    if (fMainHist!=nullptr)
    {
        if (CheckOption("cr_mainh"))
        {
            ...
            if (debug_draw) cout << "Cloning and replacing main histogram to new " << name << " at " << idx << endl;
        ...
        if (debug_draw && setXRange) cout << "x-range: " << x1 << " - " << x2 << endl;
        if (debug_draw && setYRange) cout << "y-range: " << y1 << " - " << y2 << endl;
    }
    else if (CheckOption("create_gframe"))
    {
        if (debug_draw) cout << "Creating frame" << endl;
    ...
    for (auto iObj=0; iObj<numObjects; ++iObj)
    {
        ...
        if (drawOption.Index("drawx")==0) {
            if (debug_draw)
                cout << "Skipping " << nameObj << " (" << drawOption << ")" << endl;
            continue;
        }
        ...
        if (obj->InheritsFrom(TLegend::Class())) {
            ...
            if (optimize_legend_position && legend->GetX1()==0.3 && legend->GetX2()==0.3 && legend->GetY1()==0.15 && legend->GetY2()==0.15)
            {
                ...
                legend -> SetY2(0.5);
                if (debug_draw)
                    cout << "Set legend xy12 to 0101" << endl;
            }
        }
        ...
        if (obj->InheritsFrom(TGraph::Class())) {
            if (((TGraph*)obj)->GetN()==0) {
                if (debug_draw)
                    cout << "Skipping because no points in " << nameObj << " (" << drawOption << ")" << endl;
                continue;
            }
        }
        if (obj->InheritsFrom(TCut::Class())||obj->InheritsFrom(TCutG::Class())) {
            if (debug_draw)
                cout << "Skipping cut " << nameObj << " (" << drawOption << ")" << endl;
            continue;
        }
        if (merge_pvtt_stats && obj->InheritsFrom(TPaveText::Class())) {
            listOfPaveTexts.push_back((TPaveText*)obj);
            if (debug_draw)
                cout << "Skipping " << nameObj << " to merge with stats (" << drawOption << ")" << endl;
            continue;
        }
        ...
        if (debug_draw)
            cout << "Draw: " << nameObj << " (" << drawOption0 << ") -> (" << drawOption << ")" << endl;
        obj -> Draw(drawOption);
    }
    //fRM -> Stop(12);
    ...
    {
        cout << "Trying to merge TPaveTexts to stats, but stats do not exist!!" << endl;
    }
    else {
        // This doesn't work
